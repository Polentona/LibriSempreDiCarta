...
function candidateRelationsIncomplete(candidate){
  ...
  const RETAIL_COVER_DOMAINS=['amazon.it','libraccio.it','ibs.it','mondadoristore.it','giunti.it','bancolibri.it','libreriauniversitaria.it','unilibro.it','thebanco.it'];
  ...
  async function applyCandidate(candidate,code,type){
    ...
    if(type==='isbn'&&typeof window.__LIB_RESOLVE_AUTHORITATIVE_SERIES_NEIGHBORS==='function'&&candidate.title&&candidate.author&&(!candidate.prequel||!candidate.sequel)){
      try{
        ...
        if(localRel?.authoritative){
          candidate.saga=safeSeriesName(localRel.saga);setAutoField('editSaga',candidate.saga,true)
    ...
    if(type==='isbn'&&!authoritativeRelationsResolved&&typeof window.__LIB_RESOLVE_BOUNDED_RELATIONS==='function'&&candidate.title&&candidate.author){
      try{
        ...
        if(boundedRel?.authoritative){
          candidate.saga=safeSeriesName(boundedRel.saga||'');
    ...
    if(type==='isbn'&&typeof window.__LIB_RESOLVE_UNIVERSAL_SERIES==='function'&&candidate.title&&candidate.author&&(!authoritativeRelationsResolved||candidateRelationsIncomplete(candidate))){
      try{
        ...
        if(universalRel?.saga){candidate.saga=safeSeriesName(universalRel.saga);setAutoField('editSaga',candidate.saga,true)}
    ...
    if(type==='isbn'&&candidateRelationsIncomplete(candidate)&&typeof window.__LIB_FIND_RELATIONS==='function'&&candidate.title&&candidate.author){
      try{
        ...
        if(structuredRel?.sagaChecked&&structuredRel.saga){const cleanSaga=safeSeriesName(structuredRel.saga);if(cleanSaga){candidate.saga=cleanSaga;setAutoField('editSaga',candidate.saga,true)}}
  ...
  async function enrichSavedRelations(){
    ...
    const RELATIONS_LOOKUP_VERSION=10,now=Date.now(),week=7*24*60*60*1000;
    ...
    for(const b of pending){
      try{
        const authoritativeRel=typeof window.__LIB_RESOLVE_AUTHORITATIVE_SERIES_NEIGHBORS==='function'?window.__LIB_RESOLVE_AUTHORITATIVE_SERIES_NEIGHBORS({code:b.code||b.isbn||'',title:b.title,author:b.author,saga:b.saga||''}):null;
        if(authoritativeRel?.authoritative){
          let changed=false;
          const nextSaga=String(authoritativeRel.saga||b.saga||'').trim(),nextPrequel=String(authoritativeRel.prequel||'').trim(),nextSequel=String(authoritativeRel.sequel||'').trim();
          if(String(b.saga||'').trim()!==nextSaga){b.saga=nextSaga;changed=true}
          if(String(b.prequel||'').trim()!==nextPrequel){b.prequel=nextPrequel;changed=true}
          if(String(b.sequel||'').trim()!==nextSequel){b.sequel=nextSequel;changed=true}
          b.relationsLookupAt=now;b.relationsLookupVersion=RELATIONS_LOOKUP_VERSION;saveBooks();if(changed)render();continue
        }
