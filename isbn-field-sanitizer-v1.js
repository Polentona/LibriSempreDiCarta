...
if(root.__LIB_ISBN_FIELD_SANITIZER_V1)return;root.__LIB_ISBN_FIELD_SANITIZER_V1=true;
...
function publisherParts(v){return String(v||'').split(/\s*(?:[,;|+]\s*|\s+\/\s+)\s*/).map(clean).filter(Boolean)}
function cleanPublisher(v){
  let x=clean(v).replace(/^(?:editore|publisher)\s*[:\-]?\s*/i,'').replace(/\s+(?:provider|fornitore|distributore)\s*[:\-].*$/i,'').trim();
  if(!x)return'';let parts=publisherParts(x);if(parts.length<=1)return x;
  ...
  for(const [imprint,parent] of IMPRINT_PARENT){const ii=normalized.indexOf(norm(imprint)),pi=normalized.indexOf(norm(parent));if(ii>=0&&pi>=0)keep[pi]=false}
...
function trimLeadingIdentity(s,author='',title=''){
  let x=clean(s);for(const v of [title,author]){const t=clean(v);if(!t)continue;const n=norm(t),start=norm(x.slice(0,Math.max(t.length+25,120)));if(start.startsWith(n)){const esc=t.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');x=x.replace(new RegExp('^\\s*'+esc+'\\s*[:|–—-]?\\s*','i'),'').trim()}}
  return x;
}
...
function apply(){
  ...
  if(f.publisher&&!manualPublisher){const next=cleanPublisher(f.publisher.value);if(next&&next!==clean(f.publisher.value)){f.publisher.value=next;f.publisher.dispatchEvent(new Event('change',{bubbles:true}))}}
  if(f.plot&&!manualPlot){const before=clean(f.plot.value),next=cleanPlot(before,{author:f.author?.value||'',title:f.title?.value||''});if(next.length>=60&&next!==before){f.plot.value=next;f.plot.dispatchEvent(new Event('change',{bubbles:true}))}}
}
function boot(){
  ...
  document.addEventListener('click',e=>{if(e.target?.id==='lookupMetadataBtn'){manualPublisher=false;manualPlot=false;setTimeout(apply,600);setTimeout(apply,1800)}},true);
  new MutationObserver(()=>{if(f.dlg.open){manualPublisher=false;manualPlot=false;setTimeout(apply,500)}}).observe(f.dlg,{attributes:true,attributeFilter:['open']});
  setInterval(apply,700);
}
boot();
root.__LIB_CLEAN_PUBLISHER=cleanPublisher;
root.__LIB_CLEAN_AUTOMATIC_PLOT=(v,meta)=>cleanPlot(v,meta||{});
root.__LIB_ISBN_FIELD_SANITIZER_TEST__={cleanPublisher,cleanPlot,cutCommerceTail,rawUrlNoise,markdownToText};
})();
