...
function stripWiki(v){return String(v||'').replace(/<ref\b[^>]*>[\s\S]*?<\/ref>/gi,' ').replace(/<ref\b[^/>]*\/>/gi,' ').replace(/\{\{[^{}]{0,500}\}\}/g,' ').replace(/\[\[([^\]|]+)\|([^\]]+)\]\]/g,'$2').replace(/\[\[([^\]]+)\]\]/g,'$1').replace(/''+/g,'').replace(/<[^>]+>/g,' ').replace(/&nbsp;/g,' ')}
...
function sequence(wt,title){const raw=stripWiki(wt).replace(/\r/g,''),flat=raw.replace(/\s+/g,' '),items=[];let m;
  ...
  const unique=[];for(const x of items)if(x&&!unique.some(y=>same(x,y)))unique.push(x);if(unique.length<2)return null;const idx=unique.findIndex(x=>same(x,title));if(idx<0)return null;
