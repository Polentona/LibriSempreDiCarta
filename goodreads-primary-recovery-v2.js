...
let baseGenres=null,baseSeries=null,basePlot=null;
...
async function jina(target,timeout=12000){
  for(const t of uniq([target,target.replace(/^https:\/\//i,'http://')])){const raw=await text('https://r.jina.ai/'+t,timeout);if(raw&&raw.length>80)return raw}return''
}
...
function bookInfo(raw,url){
  if(!raw)return null;const title=firstHeading(raw),author=authorFrom(raw),series=parseSeriesMeta(raw,title),seriesUrl=links(raw,'series')[0]?.url||'';
  if(!validTitle(title)&&!author)return null;
  return{source:'goodreads',title,author,publisher:'',published:'',description:cleanPlot(parseDescription(raw)),genres:goodreadsGenres(raw),cover:coverFrom(raw),series,seriesUrl,bookUrl:url||'',editionsUrl:editionsLink(raw),raw}
}
function parseAutocomplete(raw){
  raw=String(raw||'').trim().replace(/^```(?:json)?\s*/i,'').replace(/```\s*$/,'').trim();let data=null;
  ...
  const ids=[...raw.matchAll(/"(?:bookId|book_id)"\s*:\s*"?(\d+)/g)].map(m=>m[1]);return uniq(ids).map(id=>({url:'https://www.goodreads.com/book/show/'+id,title:'',cover:'',author:''}))
}
async function autocomplete(query){
  const target='https://www.goodreads.com/book/auto_complete?format=json&q='+encodeURIComponent(query),raw=await jina(target,11000);return parseAutocomplete(raw)
}
async function searchLinks(query){
  const target='https://www.goodreads.com/search?q='+encodeURIComponent(query)+'&search_type=books',raw=await jina(target,11000);return links(raw,'book')
}
...
async function loadGoodreadsBook(url,target,seed={}){
  const raw=await jina(url,12000);if(!raw)return null;let info=bookInfo(raw,url);if(!info)return null;
  let edition=null;if(info.editionsUrl)edition=await scanExactEdition(info.editionsUrl,target);
  const exact=containsIsbn(raw,target)||!!edition;if(!exact&&target)return null;
  if(edition?.url&&(!info.author||!info.genres.length||!info.description)){const eraw=await jina(edition.url,12000);const e=bookInfo(eraw,edition.url);if(e)info={...info,...e,seriesUrl:e.seriesUrl||info.seriesUrl,editionsUrl:e.editionsUrl||info.editionsUrl}}
...
async function locateGoodreads(target,seed={}){
  ...
  if(key){const direct='https://www.goodreads.com/book/isbn/'+key,raw=await jina(direct,12000);const directLinks=links(raw,'book');for(const l of directLinks.slice(0,3))add(l.url);if(containsIsbn(raw,key)&&validTitle(firstHeading(raw)))add(direct)}
  for(const r of await autocomplete(key).catch(()=>[]))add(r.url);
  for(const r of await searchLinks(key).catch(()=>[]))add(r.url);
  ...
  for(const u of candidates.slice(0,12)){const b=await loadGoodreadsBook(u,key,seed).catch(()=>null);if(b)return b}
  return null
}
...
async function resolveEnhanced(input={}){
  const code=isbn(input.code),legacy=input.legacy||{};if(!code)return api()?.resolveAll?await api().resolveAll(input):null;
  const key=code;if(cache.has(key))return cache.get(key);if(inflight.has(key))return await inflight.get(key);
  const task=(async()=>{
    const a=api();let first=a?.resolveAll?await a.resolveAll(input).catch(()=>null):null;if(usefulBaseResult(first))return first;
    ...
    const seed=await fallbackSeed(code);if(seed){gr=await locateGoodreads(code,seed).catch(()=>null);if(gr)return await buildFromGoodreads(gr,input,{...legacy,title:seed.title||legacy.title,author:seed.author||legacy.author,publisher:seed.publisher||legacy.publisher,published:seed.year||legacy.published,description:seed.description||legacy.description,cover:seed.cover||legacy.cover});sg=await storygraph({code,title:seed.title,author:seed.author}).catch(()=>null);if(sg)return fromStoryGraph(sg,{...legacy,title:seed.title,author:seed.author,publisher:seed.publisher,published:seed.year,description:seed.description,cover:seed.cover})}
...
function install(){const a=api();if(!a||root.__LIB_GOODREADS_PRIMARY_RECOVERY_INSTALLED_V2)return false;
  const original=a.resolveAll;a.resolveAll=resolveEnhanced;root.__LIB_GOODREADS_PRIMARY_BASE_RESOLVE_ALL_V2=original;
