...
async function run(){const code=norm($('editCode')?.value),ta=$('editPlot');if(!ta||ta.value.trim()||!/^97[89]\d{10}$/.test(code)||last===code)return;last=code;const my=++token;const s=$('lookupStatus');if(s){s.textContent='Sto verificando anche la descrizione italiana su Libraccio…';s.className='lookup-status busy'}const p=await lookup(code);if(my!==token||!ta||ta.value.trim())return;if(p){ta.value=p;ta.dispatchEvent(new Event('input',{bubbles:true}));if(s){s.textContent='Trama italiana recuperata da Libraccio. Controlla la bozza prima di salvare.';s.className='lookup-status ok'}}}
