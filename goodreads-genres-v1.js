(()=>{
if(window.__LIB_GOODREADS_GENRES_ONLY_V9)return;window.__LIB_GOODREADS_GENRES_ONLY_V9=true;
...
let active={sig:'',until:0,genres:[],sourceUrl:'',pending:false,lastAttempt:0};
...
function mapLabel(label){const n=norm(label),out=[],add=x=>{if(x&&!out.includes(x))out.push(x)};if(!n||NON_GENRES.has(n))return out;
  ...
  if(/^romance$|romance fiction|romantic fiction|romanzi rosa|romanzo rosa|narrativa rosa|sentimentale/.test(n))add('Narrativa rosa/sentimentale');
  ...
  if(/^new adult$/.test(n))return[];
  return out
}
async function fetchText(url,timeout=11000){for(let attempt=0;attempt<2;attempt++){const c=new AbortController(),t=setTimeout(()=>c.abort(),timeout);try{const r=await fetch(url,{signal:c.signal,headers:{Accept:'text/plain,text/html,*/*'}});if(r.ok){const text=await r.text();if(text&&text.length>250)return text}if(r.status!==429&&r.status!==503)return''}catch(e){}finally{clearTimeout(t)}if(attempt===0)await new Promise(r=>setTimeout(r,1800))}return''}
...
function labelsFromGenreSection(text){const raw=String(text||''),labels=[];
  ...
  const lines=raw.split(/\r?\n/);let start=-1;for(let i=0;i<lines.length;i++){if(/^#{1,5}\s*genres?\s*$/i.test(stripMd(lines[i]))||/^genres?\s*$/i.test(stripMd(lines[i]))){start=i+1;break}}
  if(start>=0){for(let i=start;i<Math.min(lines.length,start+80);i++){const line=lines[i];if(i>start&&/^#{1,5}\s+/.test(line))break;const x=stripMd(line);if(!x||x.length>70||/^(?:see all|genres?|community reviews?|ratings?|friends reviews?|reader q&a)$/i.test(x))continue;if(!/[A-Za-zÀ-ÿ]/.test(x))continue;labels.push(x)}}
  return uniq(labels)
}
...
async function resolveGoodreads({title,author,code}){title=clean(title);author=clean(author);const k=[norm(title),norm(author)].join('|');if(!title||!author)return{genres:[],labels:[],url:''};if(cache.has(k))return cache.get(k);const p=(async()=>{const surname=norm(author).split(' ').pop(),queries=[`site:goodreads.com/book/show \"${title}\" \"${author}\"`,code?`site:goodreads.com/book/show \"${code}\" \"${author}\"`:'' ].filter(Boolean);let candidates=[];
...
function clearNonGoodreads(){const f=document.getElementById('editCategory');if(!f)return;if(active.genres.length){const expected=uniq(active.genres).join(', ');if(f.value!==expected)writeGenres(active.genres,active.sourceUrl)}else if(active.sig&&Date.now()<active.until&&f.value){f.value='';f.dataset.genreSource='';f.dispatchEvent(new Event('input',{bubbles:true}))}}
async function run(force=false){if(active.pending)return;const d=document.getElementById('editDialog'),title=clean(document.getElementById('editTitle')?.value),author=clean(document.getElementById('editAuthor')?.value),code=clean(document.getElementById('editCode')?.value);if(!d?.open||!active.sig||Date.now()>active.until||!title||!author)return;const sig=signature();if(!sig||sig!==active.sig)return;const now=Date.now();if(!force&&now-active.lastAttempt<7000){clearNonGoodreads();return}active.lastAttempt=now;active.pending=true;try{const r=await resolveGoodreads({title,author,code});active.genres=r.genres;active.sourceUrl=r.url;if(r.genres.length)writeGenres(r.genres,r.url);else clearNonGoodreads();window.__LIB_GENRE_DELEGATE_LAST__={sig,genres:r.genres,labels:r.labels||[],source:'goodreads',url:r.url||'',field:document.getElementById('editCategory')?.value||''}}catch(e){window.__LIB_GENRE_DELEGATE_ERROR__=String(e&&e.message||e);clearNonGoodreads()}finally{active.pending=false}}
function beginLookup(){const f=document.getElementById('editCategory');if(f){f.value='';f.dataset.genreSource='';f.dispatchEvent(new Event('input',{bubbles:true}))}active={sig:'',until:Date.now()+45000,genres:[],sourceUrl:'',pending:false,lastAttempt:0};setTimeout(()=>{active.sig=signature();run(true)},650);setTimeout(()=>run(true),6000);setTimeout(()=>run(true),16000)}
function boot(){const btn=document.getElementById('lookupMetadataBtn');if(btn&&!btn.__goodreadsOnlyV9){btn.__goodreadsOnlyV9=true;btn.addEventListener('click',beginLookup,true)}if(active.sig&&Date.now()<active.until){const sig=signature();if(sig===active.sig){clearNonGoodreads();run(false)}}}
let tries=0;const timer=setInterval(()=>{tries++;boot();if(tries>=900)clearInterval(timer)},500);setTimeout(boot,0);
window.__LIB_LOOKUP_SPECIFIC_GENRES=async opts=>(await resolveGoodreads(opts||{})).genres;
window.__LIB_GOODREADS_DIRECT_V9=true;
