...
function enforce(){
  ...
  const v8=!!root.__LIB_SERIES_AUTHORITATIVE_RUNTIME_V8;
  const v7=!!root.__LIB_SERIES_AUTHORITATIVE_RUNTIME_V7;
  if(typeof base!=='function'||(!v8&&!v7&&!root.__LIB_SERIES_AUTHORITATIVE_RUNTIME_V6))return false;
  ...
  root.__LIB_SERIES_RELATION_POLICY=v8?'goodreads-order-canonical-localization-stable-v8':(v7?'goodreads-order-storygraph-localization-retry-v7':'single-owner-goodreads-complete-then-storygraph-then-wikipedia-v6');
  return true;
}
let n=0;const t=setInterval(()=>{n++;enforce();if(n>=220)clearInterval(t)},100);setTimeout(enforce,0);
