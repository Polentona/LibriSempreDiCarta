(()=>{
  if(window.__LIB_MULTI_GENRES_V2)return;
  window.__LIB_MULTI_GENRES_V2=true;
  ...
  const VERIFIED_GENRES=[
    {
      ...
      source:'Penguin Random House · Related Genres',
      verified:'2026-08-19'
    }
  ];
  ...
  function verifiedGenres(book){
    const t=norm(book?.title),a=norm(book?.author);if(!t||!a)return[];
    const hit=VERIFIED_GENRES.find(x=>norm(x.title)===t&&norm(x.author)===a);
    return hit?[...hit.genres]:[];
  }
  function bookGenres(book){
    const fromArray=Array.isArray(book?.genres)?book.genres:[];
    const legacy=fromArray.length?fromArray:(book?.category||'');
    return uniq([...parseGenreInput(legacy,{keepUnknown:true}),...verifiedGenres(book)]);
  }
  ...
  function installStyle(){
    if(document.getElementById('multiGenreV2Style'))return;
    const style=document.createElement('style');style.id='multiGenreV2Style';
    style.textContent=`
      ...
      .genre-filter.open .genre-filter-menu{display:block}
      .genre-filter-option{display:flex;align-items:center;gap:8px;padding:6px 7px;border-radius:6px;font-size:11px;cursor:pointer}
      .genre-filter-option:hover{background:#f3e4d1}.genre-filter-option input{accent-color:#607e56}
      .genre-filter-reset{width:100%;border:0;background:transparent;color:#75685d;text-align:left;padding:6px 7px;font:inherit;font-size:10px;cursor:pointer;border-bottom:1px solid #eadbc9;margin-bottom:4px}
      .genre-filter-reset:hover{color:var(--ink)}
    `;
  ...
  function patchGenreField(){
    const input=document.getElementById('editCategory');if(!input||input.__multiGenreV2Guard)return false;
    const desc=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value');if(!desc?.get||!desc?.set)return false;
    input.__multiGenreV2Guard=true;
    Object.defineProperty(input,'value',{configurable:true,get(){return desc.get.call(this)},set(v){desc.set.call(this,formatGenres(parseGenreInput(v,{keepUnknown:true})))}});
  ...
  function patchSaveBooks(){
    if(typeof saveBooks!=='function'||saveBooks.__multiGenreV2)return false;
    const base=saveBooks;
    saveBooks=function(){if(typeof books!=='undefined'&&Array.isArray(books))for(const b of books)syncBookGenres(b);return base()};
    saveBooks.__multiGenreV2=true;saveBooks.__base=base;return true;
  }

  function patchSearch(){
    if(typeof searchableBooks!=='function'||searchableBooks.__multiGenreV2)return false;
    ...
    searchableBooks.__multiGenreV2=true;searchableBooks.__base=base;return true;
  }

  function genreButtonText(){
  ...
  function renderGenreControl(box){
    ...
    const root=document.getElementById('genreFilter'),btn=document.getElementById('genreFilterBtn');
    if(btn)btn.onclick=e=>{e.stopPropagation();root?.classList.toggle('open')};
    document.querySelectorAll('[data-genre-filter]').forEach(cb=>cb.onchange=()=>{
      const g=cb.dataset.genreFilter||'';
      if(cb.checked){if(!selectedGenres.some(x=>norm(x)===norm(g)))selectedGenres.push(g)}else selectedGenres=selectedGenres.filter(x=>norm(x)!==norm(g));
      currentPage=1;if(btn)btn.textContent=genreButtonText();render();
    });
  ...
  function patchGenreFilter(){
    if(typeof getFilteredBooks!=='function'||getFilteredBooks.__multiGenreV2)return false;
    ...
    getFilteredBooks=function(){
      ...
      let list;
      try{const prev=selectedCategory;selectedCategory='';list=baseGet.apply(this,arguments);selectedCategory=prev}catch(e){list=typeof searchableBooks==='function'?searchableBooks():[]}
      if(selectedGenres.length)list=list.filter(b=>{const gs=bookGenres(b);return selectedGenres.every(sel=>gs.some(g=>norm(g)===norm(sel)))});
      if(typeof compareBooksByAuthor==='function')list=[...list].sort(compareBooksByAuthor);
      return list;
    };
    getFilteredBooks.__multiGenreV2=true;getFilteredBooks.__base=baseGet;

    if(typeof renderViewControls==='function'&&!renderViewControls.__multiGenreV2){
      ...
      renderViewControls=function(){
        ...
        const box=document.getElementById('viewControls');if(box)renderGenreControl(box);
      };
      renderViewControls.__multiGenreV2=true;renderViewControls.__base=baseControls;
    }
    if(!document.__multiGenreOutsideClick){document.__multiGenreOutsideClick=true;document.addEventListener('click',e=>{const root=document.getElementById('genreFilter');if(root&&!root.contains(e.target))root.classList.remove('open')})}
    return true;
  }

  function patchDialog(){
    if(typeof fillDialog!=='function'||fillDialog.__multiGenreV2||!document.getElementById('editCategory'))return false;
    ...
    fillDialog.__multiGenreV2=true;fillDialog.__base=baseFill;

    const form=document.getElementById('editForm');
    if(form&&typeof form.onsubmit==='function'&&!form.onsubmit.__multiGenreV2){
      ...
      const wrapped=async function(e){
        ...
        const input=document.getElementById('editCategory');if(input)input.value=formatGenres(parseGenreInput(input.value,{keepUnknown:true}));
        await baseSubmit.call(this,e);
        const dlg=document.getElementById('editDialog');if(dlg?.open)return;
        const genres=parseGenreInput(input?.value||'',{keepUnknown:true}),b=mode==='add'?books?.[0]:books?.find?.(x=>x.id==id);
        if(b){b.genres=uniq([...genres,...verifiedGenres(b)]);delete b.category;if(typeof saveBooks==='function')saveBooks();if(typeof render==='function')render()}
      };
      wrapped.__multiGenreV2=true;wrapped.__base=baseSubmit;form.onsubmit=wrapped;
    }
    return true;
  }
  ...
  async function fetchJson(url,timeout=8000){const ctrl=new AbortController(),timer=setTimeout(()=>ctrl.abort(),timeout);try{const r=await fetch(url,{signal:ctrl.signal});if(!r.ok)return null;return await r.json()}catch(e){return null}finally{clearTimeout(timer)}}
  ...
  async function enrichSavedGenres(){
    ...
    const pending=books.filter(b=>Number(b.genresLookupVersion||0)<VERSION).slice(0,10);if(!pending.length)return;
    for(const b of pending){
      try{
        const fetched=await lookupGenres(b),verified=verifiedGenres(b),merged=uniq([...bookGenres(b),...fetched,...verified]);
        b.genres=merged;delete b.category;b.genresLookupVersion=VERSION;b.genresLookupAt=Date.now();
      }catch(e){b.genres=uniq([...bookGenres(b),...verifiedGenres(b)]);delete b.category;b.genresLookupVersion=VERSION;b.genresLookupAt=Date.now()}
    }
    if(typeof saveBooks==='function')saveBooks();if(typeof render==='function')render();
    if(books.some(b=>Number(b.genresLookupVersion||0)<VERSION))setTimeout(enrichSavedGenres,900);
  }

  window.__LIB_BOOK_GENRES=bookGenres;window.__LIB_PARSE_GENRES=parseGenreInput;window.__LIB_ENRICH_GENRES=enrichSavedGenres;
  window.__LIB_VERIFIED_GENRES=VERIFIED_GENRES;
  ...
  setTimeout(()=>{boot();setTimeout(enrichSavedGenres,700)},0);
})();
