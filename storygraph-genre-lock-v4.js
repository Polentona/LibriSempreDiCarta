...
if(root.__LIB_STORYGRAPH_GENRE_LOCK_V4)return;root.__LIB_STORYGRAPH_GENRE_LOCK_V4=true;root.__LIB_STORYGRAPH_GENRE_LOCK_V5=true;
...
function state(reason=''){const i=input();root.__LIB_STORYGRAPH_GENRE_LOCK_STATE__={active,code:i.code,title:i.title,author:i.author,pending:!!active&&!stable&&!manual&&!exhausted,settled:!!stable||manual||exhausted||!active,complete:!!stable,manual,exhausted,tries,reason,at:Date.now()}}
...
function remember(i,r){if(!r?.genres?.length)return false;stable={key:keyOf(i),genres:uniq(r.genres),url:r.url||'',at:Date.now()};exhausted=false;apply();root.__LIB_STORYGRAPH_GENRE_LOCK_LAST__={input:i,...r,at:Date.now()};state('storygraph');return true}
...
async function resolve(){if(!active||inflight||manual||stable)return;const i=input();if(!i.code||!i.title||!i.author){state('waiting-metadata');return}if(consumeDelegate())return;const now=Date.now();if(now-lastTry<3800)return;if(tries>=MAX_TRIES){exhausted=true;state('exhausted');return}lastTry=now;tries++;inflight=true;state('resolving');try{const r=await resolveStoryGraph(i);if(keyOf(input())!==keyOf(i))return;if(r)remember(i,r)}catch(e){root.__LIB_STORYGRAPH_GENRE_LOCK_ERROR__=String(e&&e.message||e)}finally{inflight=false;if(!stable&&tries>=MAX_TRIES){exhausted=true;state('exhausted')}else state(stable?'storygraph':'pending')}}
function activate(){active=true;stable=null;inflight=false;lastTry=0;tries=0;manual=false;exhausted=false;successCache.clear();state('activated');setTimeout(resolve,650)}
function boot(){if(typeof document==='undefined')return;const dlg=document.getElementById('editDialog'),cat=document.getElementById('editCategory');if(!dlg||!cat){setTimeout(boot,150);return}cat.addEventListener('input',e=>{if(e.isTrusted){manual=true;state('manual')}});document.addEventListener('click',e=>{if(e.target?.id==='lookupMetadataBtn')activate()},true);for(const id of ['editCode','editTitle','editAuthor']){const el=document.getElementById(id);el?.addEventListener('input',()=>{if(dlg.open)activate()});el?.addEventListener('change',()=>{if(dlg.open)activate()})}new MutationObserver(()=>{if(dlg.open)activate();else{active=false;stable=null;manual=false;exhausted=false;state('closed')}}).observe(dlg,{attributes:true,attributeFilter:['open']});if(dlg.open)activate();setInterval(()=>{if(!active)return;if(!stable)consumeDelegate();apply();resolve()},800)}
boot();
